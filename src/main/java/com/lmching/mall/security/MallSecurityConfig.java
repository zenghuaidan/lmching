    	...
    	return new UserDetailsService() {
			...
			public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
				if("admin".equals(email)) {										
					return new org.springframework.security.core.userdetails.User("admin", "$2a$10$svgqSsLIuDS7pY6HC/8zg.3hVKXdDMoGpFp8Dy5ZxyjYzLIYyc6em", true, true, true, true, AuthorityUtils.commaSeparatedStringToAuthorityList("USER, ADMIN"));
				} else {
					User user = userService.findByEmail(email);
					return new org.springframework.security.core.userdetails.User(user.getEmail(), user.getPassword(), user.isActive(), true, true, !user.isLock(), AuthorityUtils.commaSeparatedStringToAuthorityList("USER" + (user.isAdmin() ? ", ADMIN" : "")));					
				}
			}
		};
    }
